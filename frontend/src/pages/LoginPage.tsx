...
import { Github, Code2, Award, TrendingUp, FlaskConical, Loader } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] flex flex-col items-center justify-center px-4">
      ...
      <div className="w-full max-w-md bg-[#161b22] border border-[#30363d] rounded-xl p-8 shadow-2xl">
        <h2 className="text-xl font-semibold text-[#e6edf3] mb-2">Welcome back</h2>
        ...
        {import.meta.env.DEV && (
          <div className="mt-4 pt-4 border-t border-dashed border-[#30363d]">
            ...
            >
              {devLoading ? <Loader size={16} className="animate-spin" /> : <FlaskConical size={16} />}
              Testing — skip sign in (dev only)
            </button>
            {devError && (
              <p className="text-xs text-[#f85149] text-center mt-2">{devError}</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="mt-10 grid grid-cols-3 gap-6 max-w-lg w-full">
        <Stat icon={<TrendingUp size={20} className="text-[#58a6ff]" />} value="1.5M" label="graduates/year" />
        <Stat icon={<Award size={20} className="text-[#f85149]" />} value="4.4%" label="can write working code" />
        <Stat icon={<Code2 size={20} className="text-[#3fb950]" />} value="ProofForge" label="changes that" />
      </div>
      ...
      <div className="mt-12 max-w-md w-full">
        ...
        <div className="space-y-3">
          {[
            { step: '01', label: 'Get a real bug to fix', desc: 'Real-world broken codebases, not toy examples' },
            { step: '02', label: 'Write your fix', desc: 'Monaco editor, run tests instantly' },
            { step: '03', label: 'AI code review', desc: 'Senior engineer-level feedback on your PR' },
            { step: '04', label: 'Earn your card', desc: 'Cryptographically signed portfolio proof' },
          ].map(({ step, label, desc }) => (
